...
impl MenuBar {
    ...
    pub fn consume_shortcuts(
            ...
            && fullscreen_pressed
        {
            ...
            let _ = self.event_loop.send_event(if is_fullscreen {
    ...
    pub fn show(
        ...
        egui::TopBottomPanel::top("menu_bar").show(egui_ctx, |ui| {
            egui::MenuBar::new().ui(ui, |ui| {
                ...
                ui.separator();
                ui.label("Speed  F5-/F6+/F7=1x");
                for speed in [0.25, 0.5, 1.0, 2.0, 3.0, 5.0, 10.0] {
                    let active = (playback_speed - speed).abs() < f64::EPSILON;
                    let label = format!("{speed}x");
                    if ui.selectable_label(active, label).clicked() {
                        let _ = self.event_loop.send_event(RuffleEvent::SetPlaybackSpeed(speed));
                    }
                }
            });
        });
