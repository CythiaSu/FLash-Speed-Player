...
impl RuffleGui {
    ...
    fn update(
        ...
        if show_menu {
            self.menu_bar
                .show(&locale, egui_ctx, &mut self.dialogs, player.as_deref_mut(), playback_speed);
        }

        if show_speed_overlay {
            Area::new(Id::new("speed_overlay"))
                .anchor(Align2::RIGHT_TOP, vec2(-10.0, 10.0 + menu_height_offset as f32))
                .interactable(false)
                .show(egui_ctx, |ui| {
                    Frame::new()
                        .fill(Color32::from_rgba_unmultiplied(12, 14, 18, 135))
                        .corner_radius(CornerRadius::same(6))
                        .inner_margin(Margin::symmetric(8, 4))
                        .stroke(Stroke::new(1.0, Color32::from_rgba_unmultiplied(255, 215, 80, 120)))
                        .show(ui, |ui| {
                            let speed_text = if playback_speed.fract() == 0.0 {
                                format!("{playback_speed:.0}x")
                            } else {
                                format!("{playback_speed:.2}x")
                            };
                            Label::new(
