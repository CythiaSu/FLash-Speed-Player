...
struct MainWindow {
    ...
    playback_speed: f64,
    speed_notice_until: Option<Instant>,
    event_loop_proxy: EventLoopProxy<RuffleEvent>,
}

impl MainWindow {
    fn speed_overlay_visible(&self) -> bool {
    ...
    fn set_playback_speed(&mut self, speed: f64) {
        let speed = if speed.is_finite() {
            speed.clamp(0.05, 100.0)
        } else {
            1.0
        };

        self.playback_speed = speed;
        self.speed_notice_until = if (speed - 1.0).abs() < f64::EPSILON {
            Some(Instant::now() + std::time::Duration::from_secs(2))
        } else {
            None
        };
        tracing::info!("Playback speed set to {speed:.2}x");
    }

    fn step_playback_speed(&mut self, direction: i32) {
        const SPEED_STEPS: &[f64] = &[0.25, 0.5, 1.0, 2.0, 3.0, 5.0, 10.0];
        let current = self.playback_speed;
        let idx = SPEED_STEPS
            .iter()
            .position(|speed| (*speed - current).abs() < f64::EPSILON)
            .unwrap_or_else(|| {
                SPEED_STEPS
                    .iter()
        ...
        self.set_playback_speed(SPEED_STEPS[next_idx]);
    }

    fn handle_speed_shortcut(&mut self, event: &KeyEvent) -> bool {
        ...
        match &event.logical_key {
            ...
            Key::Character(key) if key.as_ref() == "[" => {
                self.step_playback_speed(-1);
                true
            }
            Key::Character(key) if key.as_ref() == "]" => {
    ...
    fn handle_exit_shortcut(&mut self, event: &KeyEvent) -> bool {
        if event.state != ElementState::Pressed || !self.modifiers.state().control_key() {
            return false;
        }

        match &event.logical_key {
            Key::Character(key) if key.as_ref().eq_ignore_ascii_case("q") => {
                let _ = self.event_loop_proxy.send_event(RuffleEvent::ExitRequested);
                true
            }
            _ => false,
        }
    ...
    pub fn window_event(&mut self, event_loop: &ActiveEventLoop, event: WindowEvent) {
        if matches!(event, WindowEvent::RedrawRequested) {
            ...
            if !self.minimized {
                ...
                self.gui
                    .render(player, self.playback_speed, self.speed_overlay_visible());
        ...
        match event {
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                if self.handle_exit_shortcut(&event) || self.handle_speed_shortcut(&event) {
                    return;
                }
    ...
    fn about_to_wait(&mut self, gilrs: Option<&mut Gilrs>) {
        ...
        if matches!(self.loaded, LoadingState::Loaded) {
            ...
            let real_dt = new_time.duration_since(self.time);
            let dt = FloatDuration::from_std(real_dt);
            if dt.as_millis() > 0.0 {
                self.time = new_time;
                let scaled_dt = FloatDuration::from_secs(real_dt.as_secs_f64() * self.playback_speed);
                self.next_frame_time = self.player.get().map(|mut player| {
                    player.tick(scaled_dt);
...
impl ApplicationHandler<RuffleEvent> for App {
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: StartCause) {
        ...
        if cause == StartCause::Init {
            ...
            let playback_speed = self.preferences.cli.speed;
            ...
            self.main_window = Some(MainWindow {
                ...
                playback_speed,
                speed_notice_until: None,
                event_loop_proxy,
            });

            if let Some(main_window) = &mut self.main_window {
                main_window.set_playback_speed(playback_speed);
            }
        }
    }
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: RuffleEvent) {
        ...
        match (&mut self.main_window, event) {
            ...
            (Some(main_window), RuffleEvent::SetPlaybackSpeed(speed)) => {
                main_window.set_playback_speed(speed);
            }
