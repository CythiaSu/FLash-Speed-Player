...
pub struct Opt {
    ...
    /// Initial playback speed multiplier. Values above 1 speed the movie up.
    #[clap(long, default_value_t = 1.0, value_parser(parse_playback_speed))]
    pub speed: f64,
...
fn parse_playback_speed(value: &str) -> Result<f64, Error> {
    let speed: f64 = value.parse()?;
    const SPEED_STEPS: &[f64] = &[0.25, 0.5, 1.0, 2.0, 3.0, 5.0, 10.0];
    if SPEED_STEPS
        .iter()
        .any(|step| (*step - speed).abs() < f64::EPSILON)
    {
        Ok(speed)
    } else {
        Err(anyhow!(
            "Playback speed must be one of: 0.25, 0.5, 1, 2, 3, 5, 10"
        ))
    }
}
